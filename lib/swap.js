...
export function getExplorerTxLink(network, hash) {
  if (!network) {
    ...
    return `${network.explorer}/txn/${hash}?network=${testnetMode ? 'devnet' : 'mainnet'}`
  } else if (network.id.startsWith('sui')) {
    return `${network.explorer}/txblock/${hash}?network=${testnetMode ? 'devnet' : 'mainnet'}`
  } else {
...
export function getExplorerAddressLink(network, addr) {
  if (network.id.startsWith('aptos')) {
    return `${network.explorer}/account/${addr}?network=${testnetMode ? 'devnet' : 'mainnet'}`
  } else if (network.id.startsWith('sui')) {
    return `${network.explorer}/address/${addr}?network=${testnetMode ? 'devnet' : 'mainnet'}`
  } else {
...
export function getExplorerTokenLink(token) {
  if (token.link) {
    return token.link
  } else if (token.addr?.includes('::')) {
    return `account/${token.addr.split('::')[0]}?network=${testnetMode ? 'devnet' : 'mainnet'}`
  } else if (token.addr) {
