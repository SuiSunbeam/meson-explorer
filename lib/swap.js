...
export function getStatusFromEvents(events, expireTs) {
  ...
  if (maxEvent === 'RELEASED') {
    ...
    if (!events.find(e => e.name === '"RELEASED:PENDING"')) {
      return 'RELEASING...'
    } else {
      return 'RELEASING*'
    }
...
export function getExplorerTxLink(network, hash) {
  if (!network) {
    ...
    return `${network.explorer}/txblock/${hash}?network=${TESTNET ? 'testnet' : 'mainnet'}`
...
export function getExplorerAddressLink(network, addr) {
  if (network.id.startsWith('aptos')) {
    ...
    return `${network.explorer}/address/${addr}?network=${TESTNET ? 'testnet' : 'mainnet'}`
  } else if (network.id.startsWith('solana')) {
