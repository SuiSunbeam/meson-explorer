...
class ExtendedExtensionManager extends MultiExtensionManager {
  ...
  async _getMesonClient (chain) {
    ...
    let client = this.extSigner
    if (network.id.startsWith('aptos')) {
      const aptosClient = new AptosClient(network.url)
      client = adaptors.aptos.getWalletFromExtension(this.currentExt, aptosClient)
    }
    return presets.createMesonClient(network.id, client)
  }
}
